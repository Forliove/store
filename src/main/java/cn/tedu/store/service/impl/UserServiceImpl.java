...
public class UserServiceImpl
	...
	public void changeAvatar(Integer id, String avatar) throws UserNotFoundException, UpdateException {
		// 根据id查询用户信息：User findById(Integer id);
		User data = findById(id);
		// 判断查询结果是否为null
		if (data == null) {
			// 是：抛出UserNotFoundException
		...
		// 创建当前时间对象
		Date now = new Date();
		// 更新头像：updateAvatar(Integer id, String avatar, String modifiedUser, Date modifiedTime)
		updateAvatar(id, avatar, username, now);
	}
